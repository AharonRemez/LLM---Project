...
def load_weights_into_gpt(gpt, params):
    ...
    for b in range(len(params["blocks"])):
        ...
        gpt.transformer_blocks[b].att.W_query.weight = assign(gpt.transformer_blocks[b].att.W_query.weight, q_w.T)
        gpt.transformer_blocks[b].att.W_key.weight = assign(gpt.transformer_blocks[b].att.W_key.weight, k_w.T)
        gpt.transformer_blocks[b].att.W_value.weight = assign(gpt.transformer_blocks[b].att.W_value.weight, v_w.T)
        q_b, k_b, v_b = np.split((params["blocks"][b]["attn"]["c_attn"])["b"], 3, axis=-1)
        gpt.transformer_blocks[b].att.W_query.bias = assign(gpt.transformer_blocks[b].att.W_query.bias, q_b)
        gpt.transformer_blocks[b].att.W_key.bias = assign(gpt.transformer_blocks[b].att.W_key.bias, k_b)
        gpt.transformer_blocks[b].att.W_value.bias = assign(gpt.transformer_blocks[b].att.W_value.bias, v_b)
